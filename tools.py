...
def _run_adjustment(run_status):
    return (
        0.15
        if _norm(run_status) in {"run & drive", "run and drive", "run_drive"}
        else 0.0
    )
    ...
    try:
        ...
        if front_err:
            return json.dumps({
                ...
                "pricing_status": (
                    "EXCLUDED"
                    if front_err == "SEVERE_FRONT_EXCLUDED"
                    else "NOT_PRICED"
                ),
                "pricing_method": METHOD_VERSION,
                "reason": front_err,
        ...
        # Always round DOWN to nearest $25.
...
def register_tools(ctx) -> None:
    ...
    ctx.register_tool(
        ...
        description=(
            "Deterministic auction max-hammer calculator "
            "using approved PRICING_V1 rules."
        ),
    )
